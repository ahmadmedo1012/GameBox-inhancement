/*! site.bundle.js — Unified Frontend (Vanilla JS, RTL) */
(function(){
  "use strict";
  // Helpers
  ...
  // Theme boot (completes the inline pre-init)
  ...
  // Inject FAB if missing
  try {
    if (!$(".ui-theme-fab")) {
      const btn = document.createElement("button");
      btn.type = "button";
      btn.className = "ui-theme-fab";
      btn.setAttribute("aria-label", "Toggle theme");
      btn.innerHTML = '<svg width="22" height="22" viewBox="0 0 24 24" aria-hidden="true"><path d="M21.64 13A9 9 0 1111 2.36 7 7 0 0021.64 13z" fill="currentColor"/></svg>';
  ...
  try {
    const header = $("header.site-header") || $(".site-header") || $("header[role='banner']") || $("header");
    if (header) {
      const onScroll = () => {
  ...
  // Basic off-canvas (optional: data-toggle targets)
  try {
    const openers = $$("[data-ui-open]");
    const closers = $$("[data-ui-close]");
    openers.forEach(btn => btn.addEventListener("click", () => {
      const sel = btn.getAttribute("data-ui-open");
      const panel = sel ? $(sel) : null;
      if (panel) panel.classList.remove("u-hidden");
    }));
    closers.forEach(btn => btn.addEventListener("click", () => {
      const sel = btn.getAttribute("data-ui-close");
      const panel = sel ? $(sel) : null;
      if (panel) panel.classList.add("u-hidden");
    }));
  } catch(e){ /* no-op */ }

  // Ensure images are lazy if not already
  try {
    $$("img").forEach(img => {
...
/* ===== Legacy JS (concatenated) ===== */


/* File: assets/js/account.mobile.js */
// ======================================================================
...
/* =======================================================
   GameBox - app.js (page behaviors) — v4.0
   Fix: Order form uses HTML5 validity; no hardcoded field list.
   - If #orderForm has [data-preview], render receipt instead of submit.
   - Auto-sync selected radio [name="pkg"] -> hidden input (package/package_id).
   - Favorites toggle (ajax), Buy bottom sheet.
   ======================================================= */
...
(function(){
  ...
  if(orderForm){
    // Sync selected package radio to hidden field if exists
    function syncPkg(){
    ...
    orderForm.addEventListener('submit', (e)=>{
      ...
      // Preview-mode only (no server submit)
      ...
      // Otherwise, allow normal submit to server
    });
  }
...
/* ===== Dynamic order fields (category-aware) ===== */
(function(){
...
;/* === GameBox v9 Order logic (guarded) === */
...
;/* === GameBox v10 Order polish (guarded) === */
...
(function(){
  ...
  // Lazy-load للصور: استخدم data-src + class="lazy"
  ...
  // سحب أفقي مع قفل محوري (يمنع تضارب التمرير العمودي)
  function enableDragScroll(rail){
    ...
    const onMove = (e)=>{
      ...
      const x = (e.touches? e.touches[0].clientX : e.clientX);
      const dx = x - startX;
      const dy = (e.touches? e.touches[0].clientY : e.clientY) - (e.touches? e.touches[0].clientY : e.clientY); // 0 hacky no-need
      if(lockAxis === null){
        if(Math.abs(dx) > 6){ lockAxis = 'x'; }
        // لا نحتاج قفل y هنا لأن الحاوية أفقية
      }
      if(lockAxis === 'x'){
        e.preventDefault();
  ...
  // تمييز العنصر النشط في الشريط السفلي حسب الصفحة (لو ما عندك كود سابق)
  const path = (location.pathname || '').toLowerCase();
...
(function(){
  ...
  // Hook up any existing toggle buttons too
  try {
    $$("#theme-toggle,.theme-toggle,[data-theme-toggle]").forEach(btn=>{
      btn.addEventListener("click",()=>applyTheme(getTheme()==="dark"?"light":"dark"));
    });
  } catch(e){}

  // Ensure current theme is applied on DOM ready (pre-init already set, this syncs storage/attribute if needed)
  try{ applyTheme(getTheme()); }catch(e){}

  // Sticky header hide/show synchronized with optional bottom bar
  try {
    const header = $("header.site-header") || $(".site-header") || $("header[role='banner']") || $("header");
    const bottom = $(".site-bottombar") || $(".bottom-bar") || $(".mobile-bottom-bar");
    const mode = (document.body.getAttribute("data-header-mode")||"auto").toLowerCase(); // "fixed" or "auto"
    if (header) {
      ...
      const onScroll=()=>{
        ...
        if (mode==="fixed"){
          ...
          if (past && down){ header.classList.add("is-hidden"); if (bottom) bottom.classList.add("is-hidden"); }
          else { header.classList.remove("is-hidden"); if (bottom) bottom.classList.remove("is-hidden"); }
        }
        lastY=y;
        ticking=false;
      };
...
(function(){
  ...
  try{
    document.addEventListener("click", function(ev){
      ...
      const x = ev.clientX - rect.left - r/2;
      const y = ev.clientY - rect.top - r/2;
      const ripple = document.createElement("span");
      ripple.className = "ripple";
      ripple.style.cssText = "position:absolute;inset:auto;pointer-events:none;border-radius:999px;background:rgba(255,255,255,.35);width:"+r+"px;height:"+r+"px;left:"+x+"px;top:"+y+"px;transform:scale(0);opacity:.7;transition:transform .5s ease,opacity .6s ease;";
  ...
  // ESC to close panels [data-ui-panel]
  try{
    document.addEventListener("keydown", function(e){
      if(e.key !== "Escape") return;
      const openPanel = document.querySelector("[data-ui-panel]:not(.u-hidden)");
      if(openPanel) openPanel.classList.add("u-hidden");
    });
  }catch(e){}
